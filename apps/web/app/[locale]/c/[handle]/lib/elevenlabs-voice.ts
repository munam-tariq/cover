 ...
 *
 * Verbatim copy of apps/widget/src/utils/elevenlabs-voice.ts (DOM-free, framework-agnostic).
 * Kept as a copy rather than extracted to packages/shared so the widget esbuild bundle
 * stays untouched; if a third consumer appears, extract it with @elevenlabs/client as a
 * peer dependency.
 *
...
export class ElevenLabsVoiceManager {
    ...
    try {
      ...
      this.options.onError(
        err instanceof Error
          ? err
          : new Error("Failed to start ElevenLabs session")
      );
    }
  }
  ...
  private async fetchConfig(): Promise<ElevenLabsVoiceConfig | null> {
    try {
      const url = new URL(
        `${this.options.apiUrl}/api/voice/config/${this.options.projectId}`
      );
      if (this.options.visitorId)
        url.searchParams.set("visitorId", this.options.visitorId);
