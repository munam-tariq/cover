...
function ResourcesMenu({
  ...
  labels: { resources: string; quickLinks: string; recentUpdate: string };
  resourceLinks: LandingData["resourceLinks"];
}) {
    ...
    <div className="ff-resources">
      ...
      <div className="ff-resources-panel" role="menu">
        <div className="ff-resources-grid">
          <div>
            <div className="ff-resources-label">{labels.quickLinks}</div>
            {resourceLinks.map(([label, href, desc, icon]) => (
              <Link key={href} href={href} className="ff-resources-item" role="menuitem">
          ...
          {latest && (
            <Link href={"/blog/" + latest.slug} className="ff-resources-feature" role="menuitem">
              <div className="ff-resources-label">{labels.recentUpdate}</div>
...
export async function Header() {
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10 }} aria-label={t("homeAriaLabel")}>
          <Logo />
          <span
            aria-hidden="true"
            style={{
        ...
        <div className="ff-nav-links" style={{ display: "flex", alignItems: "center", gap: 30 }}>
          {nav.map(([label, href]) => (
            <Fragment key={href}>
              ...
              {href === "/#how" && <ResourcesMenu labels={menuLabels} resourceLinks={resourceLinks} />}
            </Fragment>
          ))}
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <LocaleSwitcher className="ff-locale-switcher" />
          ...
          <details className="ff-nav-details">
            ...
            <div className="ff-nav-mobile">
              {nav.map(([label, href]) => (
                <Link key={href} href={href} style={{ padding: "12px 4px", fontSize: 15, fontWeight: 500, color: "var(--ff-soft)", borderBottom: "1px solid var(--ff-line)" }}>
                  {label}
                </Link>
              ))}
              <div className="ff-nav-mobile-label">{t("resources")}</div>
              {resourceLinks.map(([label, href]) => (
                <Link key={href} href={href} style={{ padding: "12px 4px", fontSize: 15, fontWeight: 500, color: "var(--ff-soft)", borderBottom: "1px solid var(--ff-line)" }}>
                  {label}
                </Link>
              ))}
              <LocaleSwitcher className="ff-locale-switcher ff-locale-switcher-mobile" />
              <div style={{ marginTop: 12 }}>
                <Btn kind="primary" size="md" href="/login" style={{ width: "100%" }}>
                  {t("buildYourAgent")} {Ic("arrowR", { size: 16 })}
                </Btn>
              </div>
            </div>
          </details>
      ...
      <style>{`
        ...
        .ff-locale-switcher .ff-locale-btn[aria-current="true"] { color: var(--ff-ink); }
        .ff-locale-switcher-mobile { padding: 4px; }
