...
export async function CapabilitiesSection() {
  const t = await getTranslations("marketing.capabilities");
  const { caps } = getLandingData((await getLocale()) as Locale);
            ...
            <div style={{ marginTop: "auto", paddingTop: 22, display: "flex", flexDirection: "column", gap: 7 }}>
              {([["/docs", true], ["/pricing", true], ["/faq", false]] as [string, boolean][]).map(([p, hot]) => (
                ...
                >
                  ...
                  {hot && (
                    <span style={{ marginInlineStart: "auto", fontSize: 9, fontWeight: 800, color: "#fff", background: "var(--ff-accent)", padding: "2px 6px", borderRadius: 99 }}>
                      {t("cited")}
                    </span>
                  )}
                </div>
