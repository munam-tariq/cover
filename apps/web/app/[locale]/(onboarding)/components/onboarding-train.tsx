...
/** [id, icon] — label/hint come from `onboarding.train.tiles`. */
const SRC_TILES: [string, string][] = [["file", "file"], ["text", "text"], ["qa", "qa"]];
...
export function TrainStep({
  ...
  function addText() {
    ...
    onAddSource({ type: "text", name: tTitle.trim() || t("train.textSnippetDefaultName"), meta: `${tBody.trim().length} ${t("train.charsUnit")}`, content: tBody.trim() });
    setTTitle(""); setTBody(""); setOpen(null);
  }
  function addQA() {
    if (!qQ.trim() || !qA.trim()) return;
    onAddSource({ type: "qa", name: qQ.trim(), meta: t("train.tiles.qa.label"), question: qQ.trim(), answer: qA.trim() });
    setQQ(""); setQA(""); setOpen(null);
  }

  return (
    <div>
      <Heading title={t("steps.train.title")} sub={t("steps.train.sub")} />

      <div style={{ marginBottom: 22 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{t("steps.train.websiteLabel")}</label>
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".04em", color: "var(--accent)", background: "var(--accent-soft)", padding: "3px 9px", borderRadius: 99 }}>{t("steps.train.recommended")}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", height: 50, borderRadius: 12, overflow: "hidden", background: "var(--field)", border: `1.5px solid ${f ? "var(--ink)" : "var(--line-2)"}`, boxShadow: f ? "0 0 0 4px rgba(17,21,27,.06)" : "none", transition: "all .15s" }}>
          ...
          <input value={website} onChange={(e) => onWebsite(e.target.value)} onFocus={() => setF(true)} onBlur={() => setF(false)}
            placeholder={t("steps.train.websitePlaceholder")} style={{ flex: 1, height: "100%", border: "none", outline: "none", padding: "0 14px", fontSize: 15, color: "var(--text)", background: "transparent" }} />
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 9, fontSize: 13, color: "var(--muted)" }}>
          {Ic("globe", { size: 14 })} {t("steps.train.crawlHint")}
        </div>
      </div>

      <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", display: "block", marginBottom: 9 }}>{t("steps.train.otherSourcesLabel")} <span style={{ color: "var(--muted)", fontWeight: 500 }}>· {t("steps.train.optional")}</span></label>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 9 }}>
        {SRC_TILES.map(([id, ic]) => {
          const on = open === id;
          return (
            <button key={id} onClick={() => setOpen(on ? null : (id as "file" | "text" | "qa"))} style={{
              display: "flex", flexDirection: "column", gap: 7, padding: "13px 13px", borderRadius: 13, textAlign: "start", cursor: "pointer", transition: "all .15s",
              ...
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 700, color: "var(--ink)" }}>{t(`train.tiles.${id}.label`)}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1 }}>{t(`train.tiles.${id}.hint`)}</div>
              </div>
            </button>
          );
        })}
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div className="ff-onboard" style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", justifyContent: "flex-end" }}>
          ...
          <div style={{ position: "relative", width: "min(460px,92vw)", height: "100%", background: "#fff", boxShadow: isRtl ? "24px 0 70px -20px rgba(0,0,0,.35)" : "-24px 0 70px -20px rgba(0,0,0,.35)", display: "flex", flexDirection: "column", animation: `${isRtl ? "ff-drawer-in-rtl" : "ff-drawer-in"} .34s cubic-bezier(.2,.7,.2,1) both` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "18px 20px", borderBottom: "1px solid var(--line)" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15.5, fontWeight: 700, color: "var(--ink)" }}>{t(`train.drawerTitle.${open}`)}</div>
                <div style={{ fontSize: 12.5, color: "var(--muted)" }}>{t(`train.drawerSub.${open}`)}</div>
              </div>
              <button onClick={() => setOpen(null)} style={{ display: "flex", border: "none", background: "transparent", color: "var(--muted)", padding: 6 }} title={t("train.close")}>{Ic("x", { size: 18 })}</button>
            </div>
            <div className="scroll" style={{ flex: 1, overflowY: "auto", padding: 20 }}>
              {open === "file" && (
                <div onClick={() => fileRef.current && fileRef.current.click()} style={{ border: "1.5px dashed var(--line-2)", borderRadius: 14, padding: "48px 18px", textAlign: "center", cursor: "pointer", background: "#fbfbfc" }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text)" }}>{t("train.dropFile")} <span style={{ textDecoration: "underline" }}>{t("train.browse")}</span></div>
                  <div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 5 }}>{t("train.fileHint")}</div>
                  <input ref={fileRef} type="file" accept=".pdf,.txt" onChange={pickFile} style={{ display: "none" }} />
                </div>
              )}
              {open === "text" && (
                <div>
                  <label style={drawerLabel}>{t("train.titleLabel")}</label>
                  <input value={tTitle} onChange={(e) => setTTitle(e.target.value)} placeholder={t("train.titlePlaceholder")} style={{ ...field(false), height: 46, marginBottom: 16 }} />
                  <label style={drawerLabel}>{t("train.contentLabel")}</label>
                  <textarea value={tBody} onChange={(e) => setTBody(e.target.value)} rows={8} placeholder={t("train.contentPlaceholder")} style={{ ...field(false), height: "auto", padding: "12px 14px", lineHeight: 1.55 }} />
                </div>
              )}
              {open === "qa" && (
                <div>
                  <label style={drawerLabel}>{t("train.questionLabel")}</label>
                  <input value={qQ} onChange={(e) => setQQ(e.target.value)} placeholder={t("train.questionPlaceholder")} style={{ ...field(false), height: 46, marginBottom: 16 }} />
                  <label style={drawerLabel}>{t("train.answerLabel")}</label>
                  <textarea value={qA} onChange={(e) => setQA(e.target.value)} rows={7} placeholder={t("train.answerPlaceholder")} style={{ ...field(false), height: "auto", padding: "12px 14px", lineHeight: 1.55 }} />
                </div>
              )}
            </div>
            {open !== "file" && (
              <div style={{ padding: "14px 20px", borderTop: "1px solid var(--line)" }}>
                <Btn kind="primary" full onClick={open === "text" ? addText : addQA}>{Ic("plus", { size: 17 })} {open === "text" ? t("train.addSnippet") : t("train.addQaPair")}</Btn>
              </div>
            )}
          </div>
        </div>,
        ...
        <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
          {sources.map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 12px", borderRadius: 11, border: "1px solid var(--line)", background: "#fff", animation: "ff-up-sm .24s ease both" }}>
              ...
              <button onClick={() => onRemoveSource(i)} style={{ display: "flex", border: "none", background: "transparent", color: "var(--muted)", padding: 5 }} title={t("train.remove")}>{Ic("x", { size: 15 })}</button>
            </div>
          ))}
        </div>
      )}
