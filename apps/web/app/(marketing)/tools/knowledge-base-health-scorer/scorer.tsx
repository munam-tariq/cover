  ...
  const result = useMemo(() => {
    ...
    const topRecommendation =
      coverageScore === Math.min(coverageScore, freshnessScore, findabilityScore, feedbackScore)
        ? "Grow your article count — more documented answers directly increases deflection rate."
        : freshnessScore === Math.min(coverageScore, freshnessScore, findabilityScore, feedbackScore)
        ? "Update stale articles — outdated content causes wrong answers and erodes trust."
        : findabilityScore === Math.min(coverageScore, freshnessScore, findabilityScore, feedbackScore)
        ? "Add search and category structure — good content that can't be found won't deflect tickets."
        : "Set up a feedback loop — review unanswered questions weekly to close coverage gaps.";

    return { coverageScore, freshnessScore, findabilityScore, feedbackScore, total, topRecommendation };
  }, [articleCount, avgAgeDays, hasSearch, hasCategories, hasRatings, reviewsWeekly]);

  const { letter: totalLetter, color: totalColor } = scoreGrade(result.total);

  return (
    <section
      aria-label="Knowledge base health scorer"
      style={{
        display: "grid",
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--ff-ink)" }}>Score your knowledge base</div>
            <div style={{ fontSize: 13, color: "var(--ff-muted)" }}>Answer 6 questions about your support docs.</div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 20 }}>
          <div>
            ...
            <label style={{ display: "block" }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--ff-ink)", display: "block", marginBottom: 8 }}>
                How many articles / answers does your KB have?
              </span>
          ...
          <div>
            ...
            <label style={{ display: "block" }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--ff-ink)", display: "block", marginBottom: 8 }}>
                Average age of articles (months since last update)?
              </span>
              <div style={{ position: "relative" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {[
                { label: "Has a search function", value: hasSearch, set: setHasSearch },
                { label: "Has categories or topic structure", value: hasCategories, set: setHasCategories },
              ].map((item) => (
                <label
                  key={item.label}
                  style={{
          ...
          <div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {[
                { label: "Has helpfulness ratings on articles", value: hasRatings, set: setHasRatings },
                { label: "Team reviews unanswered questions weekly", value: reviewsWeekly, set: setReviewsWeekly },
              ].map((item) => (
                <label
                  key={item.label}
                  style={{
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginTop: 18 }}>
              ...
              <div style={{ marginBottom: 6 }}>
                ...
                <div style={{ fontSize: 14, color: "rgba(255,255,255,.55)" }}>out of 100</div>
              </div>
            </div>
            <div
              style={{
            ...
            >
              <span style={{ fontWeight: 800, color: "#fff" }}>Top priority: </span>
        ...
        <DimScoreCard
          ...
          recommendation={
            result.coverageScore >= 16
              ...
              ? "Add more articles. Target your most-repeated support questions first."
              : "Start by documenting your top 20 most common questions."
          }
        />
        <DimScoreCard
          icon={RefreshCw}
          ...
          recommendation={
            result.freshnessScore >= 18
              ...
              ? "Some articles may be stale. Audit content older than 12 months."
              : "Stale content creates wrong answers. Schedule a content audit now."
          }
        />
        <DimScoreCard
          icon={Search}
          ...
          recommendation={
            result.findabilityScore >= 20
              ? "Good structure. Add internal links between related articles."
              : result.findabilityScore >= 12
              ? "Add the missing element (search or categories) to improve reach."
        ...
        <DimScoreCard
          ...
          recommendation={
            result.feedbackScore >= 20
              ? "Strong loop. Act on unanswered question data within the same week."
              : result.feedbackScore >= 12
              ? "Add the missing feedback element to close the gap-detection loop."
