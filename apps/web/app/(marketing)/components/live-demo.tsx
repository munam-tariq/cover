...
export function LiveDemo({ demos = DEMOS, lock = null }: { demos?: Demo[]; lock?: number | null }) {
  ...
  useEffect(() => {
    ...
    after(2100, () =>
      typeComposer(demo.q, 34, () => {
        ...
        after(350, () => {
          setSearching(true);
          setPill("Searching knowledge base");
        });
        after(1500, () => {
          setSearching(false);
          setPill(null);
                ...
                typeComposer(demo.q2, 32, () => {
                  ...
                  after(700, () => {
                    ...
                    pushAgent(demo.a2, { lead: true }, () => {
                      ...
                      after(3800, () => {
                        if (rotating) setDi((d) => (d + 1) % demos.length);
                        else setRunId((r) => r + 1);
                      });
                    });
                  });
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: "-.01em" }}>Knowledge base</span>
        </div>
        <div className="mono" style={{ fontSize: 11.5, color: "rgba(255,255,255,.5)", marginBottom: 16 }}>
          {demo.domain} · {demo.pages.length} pages indexed
        </div>
        ...
        >
          {searching ? (
            ...
            <>
              ...
              {`Answer grounded in ${litCount} source${litCount > 1 ? "s" : ""}`}
            </>
          ) : (
            <>
              <span style={{ width: 6, height: 6, borderRadius: 99, background: "var(--ff-ok)" }} /> RAG retrieval ready
            </>
          )}
        </div>
      </div>
...
function DemoBubble({ m, demo }: { m: Msg; demo: Demo }) {
      ...
      <div style={{ alignSelf: "flex-start", maxWidth: "92%", animation: "ff-up-sm .35s ease both" }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            ...
            >
              {Ic("handoff", { size: 14 })} Talk to a human
            </span>
          </div>
        </div>
        <div
        ...
        >
          {Ic("check", { size: 13 })} Lead captured · routed to your team
        </div>
      </div>
    );
  }
  if (m.role === "user") {
    return (
      <div style={{ alignSelf: "flex-end", maxWidth: "82%", animation: "ff-in-right .3s cubic-bezier(.2,.7,.2,1) both" }}>
        <div
          style={{
            padding: "10px 14px",
    ...
    <div style={{ alignSelf: "flex-start", maxWidth: "90%", animation: "ff-up-sm .3s ease both" }}>
      ...
      {m.done && m.showCites && m.cites && (
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "var(--ff-muted)", fontWeight: 600 }}>Sources</span>
          {m.cites.map((c) => (
            <span
              key={c}
              className="mono"
