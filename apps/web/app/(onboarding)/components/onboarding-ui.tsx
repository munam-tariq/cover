...
function TestimonialView({ index }: { index: number }) {
  const t = TESTIMONIALS[Math.floor(index / 2) % TESTIMONIALS.length];
  return (
    <div key={"t" + index} style={{ animation: "ff-up .5s cubic-bezier(.2,.7,.2,1) both" }}>
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".14em", color: "rgba(255,255,255,.5)", marginBottom: 26 }}>TRUSTED BY SUPPORT TEAMS</div>
      <div style={{ background: "rgba(255,255,255,.045)", border: "1px solid rgba(255,255,255,.10)", borderRadius: 20, padding: "30px 30px 26px", maxWidth: 460, backdropFilter: "blur(6px)", boxShadow: "0 30px 60px -30px rgba(0,0,0,.6)" }}>
        <div style={{ fontWeight: 800, fontSize: 17, letterSpacing: ".01em", marginBottom: 16, color: "#fff", opacity: 0.92 }}>{t.co}</div>
        <p style={{ fontSize: 18.5, lineHeight: 1.55, fontWeight: 500, letterSpacing: "-.01em", color: "rgba(255,255,255,.95)" }}>&ldquo;{t.quote}&rdquo;</p>
        <div style={{ marginTop: 22, display: "flex", alignItems: "center", gap: 11 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,.55)" }}>{t.role}, {t.co}</div>
          </div>
        </div>
      </div>
    </div>
...
function FeaturesView() {
  return (
    <div key="features" style={{ animation: "ff-up .5s cubic-bezier(.2,.7,.2,1) both", maxWidth: 470 }}>
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".14em", color: "rgba(255,255,255,.5)", marginBottom: 26 }}>WHAT YOUR AGENT DOES</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {FEATURES.map(([ic, title, desc]) => (
          <div key={title} style={{ display: "flex", gap: 15, alignItems: "flex-start", padding: "15px 16px", borderRadius: 16, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.09)" }}>
            ...
            <div>
              <div style={{ fontSize: 15.5, fontWeight: 700, color: "#fff", letterSpacing: "-.01em" }}>{title}</div>
              <div style={{ fontSize: 13.5, lineHeight: 1.5, color: "rgba(255,255,255,.6)", marginTop: 3 }}>{desc}</div>
            </div>
          </div>
        ))}
      </div>
...
function ProofPanel({ index }: { index: number }) {
  ...
  return (
    <div style={{ position: "relative", height: "100%", overflow: "hidden", background: "linear-gradient(160deg,#11151b 0%,#161c27 54%,#10141b 100%)", color: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "absolute", top: "-14%", right: "-12%", width: 440, height: 440, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,255,255,.12), transparent 66%)", filter: "blur(6px)" }} />
