...
const OB_ICONS: Record<string, string> = {
  ...
  globe: "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18ZM3 12h18M12 3c2.4 2.5 3.6 6 3.6 9s-1.2 6.5-3.6 9c-2.4-2.5-3.6-6-3.6-9s1.2-6.5 3.6-9Z",
  ...
  thumbDown: "M17 13.5V4h3v9.5zM17 13.5l-3.6 6.6c-1.2 0-2-.8-2-2v-3.1H6.7a2 2 0 0 1-2-2.3l1-5.4a2 2 0 0 1 2-1.7H17",
  ...
  sources: "M4 7a8 8 0 0 1 8 0 8 8 0 0 1 8 0v11a8 8 0 0 0-8 0 8 8 0 0 0-8 0zM12 7v11",
};
...
function Icon({ d, size = 18, sw = 1.7, fill = "none", style }: IconProps & { d: string }) {
  return (
...
export const Ic = (key: string, p: IconProps = {}) => <Icon d={OB_ICONS[key]} {...p} />;

/* ---------------- step model ---------------- */
export const RAIL_STEPS = ["welcome", "hear", "size", "goal", "name", "train"] as const;

/** Icon key per knowledge source type. */
export const sourceIcon: Record<string, string> = { website: "globe", file: "file", text: "text", qa: "qa", url: "globe" };

export const HEAR_OPTIONS: [string, string][] = [
  ["search", "Google search"], ["community", "Online community"], ["linkedin", "LinkedIn"],
  ["x", "X (Twitter)"], ["youtube", "YouTube"], ["friend", "A friend or colleague"],
  ["ai", "AI recommendation"], ["other", "Other"],
];

export const SIZE_OPTIONS: [string, string, string][] = [
  ["startup", "Startup", "1–9"], ["small", "Small business", "10–49"],
  ["mid", "Mid-market", "50–499"], ["enterprise", "Enterprise", "500+"],
];

export const GOAL_OPTIONS: [string, string, string, string][] = [
  ["answer", "scan", "Answer customer questions", "Deflect FAQs and support tickets 24/7."],
  ["leads", "users", "Capture & qualify leads", "Collect details and screen intent before handoff."],
  ["handoff", "handoff", "Route to my team", "Hand warm conversations to a human, seamlessly."],
...
export const FEATURES: [string, string, string][] = [
  ...
  ["users", "Captures & qualifies leads", "Collects details and intent, then routes the warm ones to you."],
  ["handoff", "Human handoff, built in", "Escalates to your team the moment a person is needed."],
  ["bolt", "Live in minutes", "Point it at your website and it's ready to answer."],
];

/* ---------------- shared style fragments ---------------- */
export const card: React.CSSProperties = {
  background: "#fff", border: "1px solid var(--line)", borderRadius: 16,
  padding: "16px 17px", boxShadow: "0 2px 8px -4px rgba(16,24,40,.08)",
};
export const cardLabel: React.CSSProperties = {
  fontSize: 11, fontWeight: 700, letterSpacing: ".1em", color: "var(--muted)", marginBottom: 12,
};
...
export const field = (focused: boolean): React.CSSProperties => ({
  width: "100%", height: 50, padding: "0 15px", borderRadius: 12, fontSize: 15, color: "var(--text)",
  background: "var(--field)", outline: "none", transition: "border-color .15s, box-shadow .15s",
  ...
  return (
    <div
      ...
      style={{
        ...
        ["--lt" as string]: "rgba(17,21,27,.04)", ["--lt-size" as string]: "70px",
        maskImage: "radial-gradient(120% 100% at 50% 50%, #000 35%, transparent 82%)",
        WebkitMaskImage: "radial-gradient(120% 100% at 50% 50%, #000 35%, transparent 82%)",
      }}
    />
  );
}
